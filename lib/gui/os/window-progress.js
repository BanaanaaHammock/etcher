 ...
 * @param {String} status - flashing/validating status
 *
 * @example
 * windowProgress.set(85)
 */
exports.set = (percentage, status) => {
  exports.currentWindow.setProgressBar(utils.percentageToFloat(percentage))
  const statusString = status === 'check' ? 'Validating' : 'Flashing'
  const dash = '\u2013'
  exports.currentWindow.setTitle(`${packageJSON.displayName} ${dash} ${statusString} ${percentage}%`)
}
...
exports.clear = () => {
  ...
  exports.currentWindow.setTitle(packageJSON.displayName)
}
