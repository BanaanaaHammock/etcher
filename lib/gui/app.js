...
app.run(() => {
  store.subscribe(() => {
    ...
    const flashProgress = messages.info.flashProgress(currentFlashState)
    const title = `${packageJSON.displayName} ${DASH_UNICODE_CHAR} ${flashProgress}`
    windowProgress.setTitle(title)
  })
})
