...
app.run(() => {
  store.subscribe(() => {
    ...
    const statusString = currentFlashState.type === 'check' ? 'Validating' : 'Flashing'
    const DASH_UNICODE_CHAR = '\u2013'
    const title = `${packageJSON.displayName} ${DASH_UNICODE_CHAR} ${statusString} ${percentage}%`
    windowProgress.set(currentFlashState.percentage, title)
  })
})
